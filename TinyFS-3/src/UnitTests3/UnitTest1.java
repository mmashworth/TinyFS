...
public class UnitTest1 {
	...
	public static void test1(ClientFS cfs){
		...
		for(int i = 1; i <= N; i++){
			fsrv = cfs.CreateDir(ClientFS.root + "/" + dir1 + "/", String.valueOf(i));
			if( fsrv != FSReturnVals.Success ){
				System.out.println("Unit test 1 result: fail!2");
	    		return;
			}
			gen1[i - 1] = ClientFS.root + "/" + dir1 + "/" + i;
		}
		
		String[] ret1 = cfs.ListDir(ClientFS.root + "/" + dir1);
		...
		fsrv = cfs.CreateDir(ClientFS.root + "/", dir2);
		if( fsrv != FSReturnVals.Success ){
		...
		String prev = "/" + dir2;
		for(int i = 1; i <= N; i++){
			fsrv = cfs.CreateDir(ClientFS.root + prev + "/", String.valueOf(i));
			if( fsrv != FSReturnVals.Success ){
				System.out.println("Unit test 1 result: fail!5");
	    		return;
			}
			prev = prev + "/" + i;
			gen2[i - 1] = ClientFS.root + prev;
		}	
		
		ret1 = cfs.ListDir(ClientFS.root + "/" + dir2);
